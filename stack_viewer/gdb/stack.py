...
class StackContext:
    ...
    def _selected_inferior(self):
        try:
            inferior = gdb.selected_inferior()
        except gdb.error:
            return None
        return inferior if inferior and getattr(inferior, "is_valid", lambda: True)() else None
    ...
    def _parse_register(self, name: str) -> Optional[int]:
        try:
            return int(gdb.parse_and_eval(f"${name}"))
        except gdb.error:
            return None

    def _frame_pc(self) -> Optional[int]:
        if self.frame is None:
            return None
        try:
            return int(self.frame.pc())
        except gdb.error:
            return None

    def _frame_name(self) -> Optional[str]:
        if self.frame is None:
            return None
        try:
    ...
    def sync(self) -> StackSnapshot:
        self.inferior = self._selected_inferior()
        self.thread = self._selected_thread()
        self.frame = self._selected_frame()
        self.pointer_size = self._get_pointer_size()
        self.rsp = self._read_stack_pointer()

        self.snapshot = StackSnapshot(
            stack_pointer=self.rsp,
            pointer_size=self.pointer_size,
            frame_pc=self._frame_pc(),
            frame_sp=self.rsp,
            function_name=self._frame_name(),
            view_direction="up",
        )
    ...
    def _read_stack_pointer(self) -> Optional[int]:
        for register_name in ("rsp", "sp"):
            value = self._parse_register(register_name)
    ...
    def read_memory(self, addr: int, size: int) -> ByteBlock:
        if self.inferior is None:
            return ByteBlock(address=addr, error="No valid GDB inferior selected")

        try:
            data = self.inferior.read_memory(addr, size)
        except gdb.MemoryError as exc:
            return ByteBlock(address=addr, error=str(exc))
        except gdb.error as exc:
            return ByteBlock(address=addr, error=str(exc))

        return ByteBlock(address=addr, data=bytes(data))
    ...
    def read_string(self, addr: int, max_len: int = 100) -> Optional[str]:
        block = self.read_memory(addr, max_len)
        if block.error or not block.data:
            return None

        raw = block.data.split(b"\x00", 1)[0]
        if not raw:
            return None
    ...
    def get_symbol(self, addr: int) -> Optional[str]:
        try:
            output = gdb.execute(f"info symbol {addr:#x}", to_string=True).strip()
        except gdb.error:
            return None

        if not output or output.startswith("No symbol"):
            return None
        return output
